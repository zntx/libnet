...
//    }
    ...
    virtual Slice<T> slice(size_t start, size_t end) {
        if (start >= this->len)
        ...
        return Slice(this->addr + start, end - start);
    }

    virtual Slice slice(size_t pos) {
        if (pos >= this->len)
            return Slice(nullptr, 0);

        return Slice(this->addr + pos, this->len - pos);
    }
